...
public class ControlBD {
	...
	private static final String[] camposEncargadoServicioSocial = new String[] {
			"idEncargado", "nombre", "email", "telefono", "facultad",
			"escuela", "path" };
		...
		public void onCreate(SQLiteDatabase db) {
			...
			try {
				...
				db.execSQL("create table ENCARGADOSERVICIOSOCIAL ( IDENCARGADO INTEGER not null primary key autoincrement, NOMBRE VARCHAR(100) not null,"
						+ " EMAIL VARCHAR(50),TELEFONO VARCHAR(8) not null, FACULTAD VARCHAR(100),ESCUELA CHAR(100), PATH VARCHAR(100), ENVIADO BOOLEAN NOT NULL);");// agregado
																																										// boleano
				...
				db.execSQL("create table PROYECTO ( IDPROYECTO INTEGER not null primary key autoincrement, IDSOLICITANTE INTEGER not null, IDTIPOPROYECTO INTEGER "
						+ "not null, IDENCARGADO INTEGER not null, NOMBRE VARCHAR(100) not null, ENVIADO BOOLEAN NOT NULL, constraint FK_PROYECTO_ADMINISTR_ENCARGAD foreign key "
				...
				db.execSQL("create table TIPOPROYECTO ( IDTIPOPROYECTO INTEGER not null primary key autoincrement, NOMBRE VARCHAR(100) not null, ENVIADO BOOLEAN NOT NULL );");
				...
				db.execSQL("insert into tipoproyecto values (null, 'Gubernamental','false');");
				db.execSQL("insert into tipoproyecto values (null, 'Social','false');");
				db.execSQL("insert into tipoproyecto values (null, 'Privado','false');");
				...
				db.execSQL("insert into encargadoserviciosocial values(null, 'Esteban Gonzalez', 'gonzalez@ues.edu.sv', '23458512', 'Economía', 'Economía','','false');");// estado
																																											// sin
																																											// enviar
				...
				db.execSQL("insert into proyecto values(null, 2, 2, 3, 'Cuidado de perrros','false');");
				db.execSQL("insert into proyecto values(null, 1, 3, 2, 'Atención a personas','false');");
	...
	public String insertar(AsignacionProyecto asignacion) {
		...
		try {
			contador = db.insertOrThrow("asignacionproyecto", null,
					valoresAsignacion);
	...
	public String insertar(Institucion institucion) {
		String mensaje = "";
		Institucion institucionVerificando = consultarInstitucion(institucion
				.getNit());
		// long contador = 0;
	...
	public String insertar(Proyecto proyecto) {
		...
		values.put("enviado", proyecto.getEnviado());
	...
	public String insertar(Solicitante solicitante) {
		...
		/*
		 * if (contador == -1 || contador == 0) mensaje =
		 * "Error al Insertar el registro. Verificar inserción"; else mensaje =
		 * "Registro ingresado " + contador;
		 */
		String[] arrayEmail = { solicitante.getCorreo() };
		Cursor cursor = db.query("solicitante", camposSolicitante,
				"correo_electronico = ?", arrayEmail, null, null, null);
		cursor.moveToFirst();
	...
	public String actualizar(AsignacionProyecto asignacion) {
		...
		contador += db.update("asignacionproyecto", cv,
				"carnet = ? AND idproyecto=?", id);
	...
	public String actualizar(Solicitante solicitante) {
		...
		Log.i("DB",
				"Se almacena: " + solicitante.getNombre() + " "
	...
	public String actualizar(TipoTrabajo tipoTrabajo) {
		...
		valoresTipoTrabajo.put("valor", tipoTrabajo.getValor());
		try {
			contador = db.update("tipotrabajo", valoresTipoTrabajo,
					"idtipotrabajo=?", id);
		} catch (Exception integridad) {
			String ex = integridad.getMessage();
			return ex.substring(0, ex.lastIndexOf("(code 19)"));
	...
	public String eliminar(AsignacionProyecto asignacion) {
		...
		int contador = 0;
		String id[] = { asignacion.getCarnet(), asignacion.getIdProyecto() };
		contador += db.delete("asignacionproyecto",
				"carnet=? AND idproyecto=?", id);
		if (contador == 0 || contador == -1) {
	...
	public String eliminar(EncargadoServicioSocial encargado) {
		String regAfectados = "";
		int contador = 0;
		contador += db.delete("encargadoserviciosocial", "idencargado='"
	...
	public String eliminar(TipoProyecto tipoProyecto) {
		...
		contador += db.delete("tipoproyecto",
				"idtipoproyecto='" + tipoProyecto.getIdTipoProyecto() + "'",
				null);
		regAfectados += contador;
		if (contador == 0 || contador == -1)
			return regAfectados;
	...
	public String eliminar(TipoTrabajo tipoTrabajo) {
		String regAfectados = "filas afectadas= ";
		long contador = 0;
		contador += db.delete("tipotrabajo",
				"idtipotrabajo='" + tipoTrabajo.getIdTipoTrabajo() + "'", null);
		regAfectados += contador;
		if (contador == 0 || contador == -1)
	...
	public ArrayList<AsignacionProyecto> consultarAsignacionProyecto(
			String parametro, int tipo) {
	...
	public Bitacora consultarBitacora(String parametro) {
		...
		Cursor cursor = db.query("bitacora", camposBitacora, "id= ?", id, null,
				null, null);
		if (cursor.moveToFirst()) {
	...
	// ///////Metodo para que regrese mas de un dato en la consulta
	...
	public ArrayList<Cargo> consultarCargo(String busqueda, int seleccion) {
		...
		case 1:
			...
			cursor = db.query("cargo", camposCargo, "nombre= ?", id, null,
					null, null);
			break;
		// Se eligio email
	...
	public Institucion consultarInstitucion(String nitInstitucion) {
		...
		Cursor cursor = db.query("institucion", camposInstitucion, "nit = ?",
				nit, null, null, null);
	...
	public Institucion consultarInstitucionById(String idInstitucion) {
		...
		Cursor cursor = db.query("institucion", camposInstitucion,
				"idinstitucion = ?", id, null, null, null);
	...
	/*
	 * public Cargo consultarCargo(String idCargo){ Cargo cargo = new Cargo();
	 * cargo.setDescripcion("descripcion"); cargo.setIdCargo("0");
	 * cargo.setNombre("Supervisor"); return cargo; }
	 */
	...
	public Solicitante consultarSolicitante(String idSolicitante) {
		...
		if (cursor.moveToFirst()) {
			Solicitante solicitante = new Solicitante(cursor.getString(1),// IdInstitucion
					...
					cursor.getString(5), cursor.getString(6)); // email

			solicitante.setIdSolicitante(idSolicitante);
			Log.i("OK", "Retornado solicitante: " + cursor.getString(0) + " "
	...
	public TipoTrabajo consultarTipoTrabajo(String idTipoTrabajo) {
		...
		if (cursor.moveToFirst()) {
			...
			objTipoTrabajo.setIdTipoTrabajo(toString()
	...
	public ArrayList<Alumno> consultarAlumnoNoEnviado() {
		...
		String[] valor = { "false" };
		Cursor cursor;
		cursor = db.query("alumno", camposAlumno, "enviado = ?", valor, null,
				null, null);
		if (cursor.moveToFirst()) {
			do {
				Alumno alumno = new Alumno();
	...
	public ArrayList<Proyecto> consultarProyectoNoEnviado() {
		ArrayList<Proyecto> proyectosNoEnviados = new ArrayList<Proyecto>();
		String[] valor = { "false" };
		Cursor cursor;
		cursor = db.query("proyecto", camposProyecto, "enviado = ?", valor,
				null, null, null);
		if (cursor.moveToFirst()) {
			do {
				...
				proyecto.setIdEncargado(cursor.getInt(3));
				proyecto.setNombre(cursor.getString(4));
				proyectosNoEnviados.add(proyecto);
			} while (cursor.moveToNext());
			return proyectosNoEnviados;
		} else {
			return null;
		}
	}
	...
	public ArrayList<EncargadoServicioSocial> consultarEncargadoNoEnviado() {
		...
		Cursor cursor;
		cursor = db.query("encargadoserviciosocial",
				camposEncargadoServicioSocial, "enviado = ?", valor, null,
				null, null);
		if (cursor.moveToFirst()) {
			do {
				EncargadoServicioSocial encargado = new EncargadoServicioSocial();
	...
	public String verificarUsuario(String usuario, String password) {
		...
		String[] campo = { "tipo" };
		Cursor cursor = db.query("usuarios", campo,
				"usuario = ? AND password = ?", id, null, null, null);
		if (cursor.moveToFirst()) {
			return cursor.getString(0);
		} else {
			return null;
	...
	private boolean verificarIntegridad(Object dato, int relacion)
		...
		case 2: {// verifica si existe el proyecto antes de actualizar ,
					// eliminar
			// tambien para actualizar tendria que verificar que exista ese
			// tipoproyecto
	...
	public String obtenerFechaActualizacion(String tabla) {
		...
		String[] valor = { tabla };
		String[] campo = { "fecha" };
		Cursor cursor = db.query("actualizaciones", campo, "tabla = ?", valor,
				null, null, null);
		if (cursor.moveToFirst()) {
			return cursor.getString(0);
		} else {
	...
	public void establecerEncargadoEnviado(int id) {
		...
		valores.put("enviado", "true");
		try {
			contador = db.update("encargadoserviciosocial", valores,
					"idencargado = ?", valor);
		} catch (Exception integridad) {
			Log.v("enviado", integridad.getMessage());
		}
